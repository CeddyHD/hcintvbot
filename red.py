...
try:
    ...
    print("Mr. HR needs Python 3.5 or superior.\n"
          "Consult the guide for your operating system "
          "and do ALL the steps in order.\n")
    sys.exit()
...
def initialize(bot_class=Bot, formatter_class=Formatter):
    ...
    async def on_ready():
        ...
        print("-----------------")
        print("Mr. HR - Interview Bot of HoundCast")
        print("-----------------")
        print(str(bot.user))
        print("\nConnected to:")
...
def interactive_setup(settings):
    ...
    if first_run:
        print("Mr. HR - First run configuration\n")
...
def load_cogs(bot):
    ...
    if owner_cog is None:
        print("The owner cog is missing. It contains core functions without "
              "which Mr. HR cannot function. Reinstall.")
        exit(1)
